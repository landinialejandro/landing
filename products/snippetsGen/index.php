<?php
include("../header.php");
?>

<div class="col-md-4 title-panel">
    <div class="title-text-panel">
        <h1><a href="<?php echo $home; ?>">LANDINI</a></h1>
        <p>
            Landini Snippets generator<br>
            Para <a href="https://code.visualstudio.com/docs/editor/userdefinedsnippets" target="_blank">VSCode</a><br>
        </p>
    </div>
    <div>
        <img class="rounded" src="<?php echo $logo; ?>" alt="">
    </div>
</div>

<div class="col-md-4 feature-panel">
    <h2>La mejor generador de Snippets gratis para VSCode!</h2>
    <p>Desarrollado en JS y HTML.<br>
        Obten la mejor experiancia del tu entorno preferido para desarrollo.
        Genera de manera sencilla todos tus snippets!
        <br>
        <ul>
            <li>
                <h3> Puedes utilizarlo desde <a href="../../sys/snippetsGen/index.html">aqui</a> gratuitamente</h3>
            </li>
            <li>
                <h3>Descarga y usalo en tu PC off-line!</h3>
                <a href="https://github.com/landinialejandro/vscode_genSnippets/">desde gitHub</a>
            </li>
            <li>
                <h3>3 en 1 - Multiple formas de editar!</h3>
                <p>Formulario ágil para generar nuevos conjuntos de Snippets</p>
                <p>Pega tus snippets que ya posees y editalos de manera mucho más visual</p>
            </li>
        </ul>
    </p>

</div>
<?php
include("../footer.php");
?>